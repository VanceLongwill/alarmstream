...
export default class TimeInput extends Component {
  ...
  handleHoursChange = (event) => {
    ...
    const updatedHoursMoment = this.state.time.clone();
  ...
  handleMinsChange = (event) => {
    ...
    const updatedMinsMoment = this.state.time.clone();
    ...
    if (secondAndThird < 60) {
      ...
      validatedMins = secondAndThird;
    } else if (firstAndSecond < 60) {
      validatedMins = firstAndSecond;
    } else {
      validatedMins = 0;
    }
    this.setState({
      time: updatedMinsMoment.set('minutes', validatedMins),
    });
  }
